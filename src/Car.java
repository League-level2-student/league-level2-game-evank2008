...
public class Car {
	...
	void draw(Graphics g) {
		...
		// g.drawRect(collisionHull.x, collisionHull.y, collisionHull.width,
		// collisionHull.height);
		g.setColor(Color.PINK);
		// predictor of destination
		if (!hasThisCarCrashed) {
			if (momentumX + getXAccel() >= 0) {
				g.drawLine(x + 159, y + 27, x + 159 + (momentumX + getXAccel()) * gridSpeedPx,
						y + 27 + (momentumY + getYAccel()) * gridSpeedPx);
			} else {

				g.drawLine(x, y + 27, x + (momentumX + getXAccel()) * gridSpeedPx,
						y + 27 + (momentumY + getYAccel()) * gridSpeedPx);
			}
		}
	}
